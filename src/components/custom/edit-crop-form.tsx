...
import { parseMonthString, type CropType, type Crop, ALL_MONTHS, type RegionName } from "@/lib/crop-data";
import { useCropData } from "@/context/crop-data-context";
import { useRouter } from "next/navigation";
import { useToast } from "@/hooks/use-toast";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Pencil, Sparkles, Loader2, CalendarIcon } from "lucide-react";
...
const editCropFormSchema = z.object({
...
export type EditCropFormValues = z.infer<typeof editCropFormSchema>;

interface EditCropFormProps {
  initialCropData: Crop;
}

export function EditCropForm({ initialCropData }: EditCropFormProps) {
  const { editCrop, regions } = useCropData(); 
  ...
  const form = useForm<EditCropFormValues>({
    resolver: zodResolver(editCropFormSchema),
    defaultValues: {
      name: initialCropData.name,
      region: initialCropData.region,
      plantingMonthsStr: initialCropData.plantingMonths.join(', '),
      weedingInfo: initialCropData.weedingInfo,
      harvestMonthsStr: initialCropData.harvestMonths.join(', '),
      type: initialCropData.type,
      notes: initialCropData.notes,
      iconHint: initialCropData.iconHint || "",
      datePlanted: initialCropData.datePlanted ? new Date(initialCropData.datePlanted) : undefined,
    },
  });
  ...
  function onSubmit(data: EditCropFormValues) {
    const updatedCrop: Crop = {
      id: initialCropData.id, 
      name: data.name,
      ...
      uploadedImageDataUri: initialCropData.uploadedImageDataUri, // Preserve existing image
    };
    editCrop(updatedCrop);
    toast({
      title: "Crop Updated",
      description: `${data.name} has been successfully updated for Uganda.`,
    });
    router.push("/");
  }
    ...
    <Card className="w-full max-w-2xl mx-auto shadow-xl">
      <CardHeader>
        <div className="flex items-center space-x-2">
          ...
          <CardTitle>Edit Crop: {initialCropData.name} (in Uganda)</CardTitle>
        </div>
        <CardDescription>Update the details below. You can also use AI to help autofill some fields.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">
              <Pencil className="mr-2 h-4 w-4" /> Update Crop Details
            </Button>
          </form>
        </Form>
      </CardContent>
